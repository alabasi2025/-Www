...
@Component({
  selector: 'app-repmemo',
  imports: [CommonModule, DecimalPipe, DatePipe, FormsModule, LegacyToolbarComponent, LegacyStatusBarComponent],
  templateUrl: './repmemo.component.html',
  styleUrl: './repmemo.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RepmemoComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('REPMEMO');

  readonly loading = signal(false);
  readonly err = signal<string | null>(null);
  readonly rows = signal<Row[]>([]);
  readonly searched = signal(false);
  readonly paramQ = signal('');
  readonly paramAmount = signal('');
  ...
  async runReport() {
    ...
    try {
      const w: string[] = [];
      if (this.paramQ()) w.push("UPPER(MEMOS) LIKE UPPER('%" + this.paramQ() + "%')");
      if (this.paramAmount()) w.push('(MDIN=' + this.paramAmount() + ' OR DAN=' + this.paramAmount() + ')');
