...
@Component({
  selector: 'app-rsedifm',
  imports: [CommonModule, DecimalPipe, DatePipe, FormsModule, LegacyToolbarComponent, LegacyStatusBarComponent],
  templateUrl: './rsedifm.component.html',
  styleUrl: './rsedifm.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RsedifmComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('RSEDIFM');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/AMHSB?limit=500' + where + '&orderBy=NOS DESC'));
