...
@Component({
  ...
  templateUrl: './rtsm.component.html',
  styleUrl: './rtsm.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RtsmComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('RTSM');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/TSMK?limit=500' + where + '&orderBy=NOS DESC'));
