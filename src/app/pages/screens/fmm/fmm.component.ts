...
@Component({
  ...
  templateUrl: './fmm.component.html',
  styleUrl: './fmm.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class FmmComponent implements OnInit {
  private http = inject(HttpClient);
  private perms = inject(PermissionService);
  readonly p = this.perms.forScreen('FMM');

  readonly model = signal({ DATES: '', NOA: 0, NAMES: '', MEMOS1: '', NOAML: 1, SARSF: 1, MRT: 0, TYPEMS: 9 });
  ...
  async loadList() {
    ...
    try {
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/FMM?limit=500&orderBy=NOS DESC'));
  ...
  async loadOne(nos: number) {
    this.loading.set(true);
    try {
      const mR = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>(`/api/data/FMM?where=NOS=${nos}&limit=1`));
      if (mR.ok && mR.rows[0]) {
        const m = mR.rows[0]; this.masterRaw.set(m);
        this.model.set({ DATES: String(m['DATES']??'').slice(0,10), NOA: Number(m['NOA']??0), NAMES: String(m['NAMES']??m['NAMEA']??''), MEMOS1: String(m['MEMOS1']??''), NOAML: Number(m['NOAML']??1), SARSF: Number(m['SARSF']??1), MRT: Number(m['MRT']??0), TYPEMS: 9 });
        const dR = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>(`/api/data/FMMF?where=NOS=${nos}&orderBy=RECNO`)); this.details.set(dR.rows||[]);
  ...
  onToolbarAction(action: LegacyToolbarActionId) {
    switch(action) {
      case 'new': this.mode.set('new'); this.model.set({DATES:new Date().toISOString().slice(0,10),NOA:0,NAMES:'',MEMOS1:'',NOAML:1,SARSF:1,MRT:0,TYPEMS:9}); this.masterRaw.set({}); this.details.set([]); this.clearMessages(); break;
  ...
  async save() {
    ...
    try {
      ...
      const r = await firstValueFrom(isNew ? this.http.post<{ok:boolean;error?:string}>('/api/data/FMM',body) : this.http.put<{ok:boolean;error?:string}>('/api/data/FMM',{...body,_where:'NOS='+this.masterRaw()['NOS']}));
  ...
  async del() {
    ...
    try { await firstValueFrom(this.http.delete('/api/data/FMM?where=NOS='+nos)); await this.loadList(); }
    catch(e){this.err.set(e instanceof Error?e.message:'خطأ');} this.saving.set(false);
  }
