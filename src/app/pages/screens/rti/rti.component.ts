...
export class RtiComponent {
  ...
  readonly p = inject(PermissionService).forScreen('RTI');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/ATM?limit=500' + where + '&orderBy=NOS DESC'));
