...
@Component({
  ...
  templateUrl: './repfbm.component.html',
  styleUrl: './repfbm.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RepfbmComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('REPFBM');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/FBM?limit=500' + where + '&orderBy=NOS DESC'));
