...
@Component({
  ...
  templateUrl: './rmm.component.html',
  styleUrl: './rmm.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RmmComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('RMM');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/FMM?limit=500' + where + '&orderBy=NOS DESC'));
