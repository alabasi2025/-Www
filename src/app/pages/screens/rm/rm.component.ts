...
export class RmComponent {
  ...
  readonly p = inject(PermissionService).forScreen('RM');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/FM?limit=500' + where + '&orderBy=NOS DESC'));
