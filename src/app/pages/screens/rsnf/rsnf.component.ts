...
@Component({
  ...
  templateUrl: './rsnf.component.html',
  styleUrl: './rsnf.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RsnfComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('RSNF');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/DATA_AG?limit=500' + where + '&orderBy=NOS DESC'));
