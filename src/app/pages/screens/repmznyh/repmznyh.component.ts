...
@Component({
  selector: 'app-repmznyh',
  imports: [CommonModule, DecimalPipe, DatePipe, FormsModule, LegacyToolbarComponent, LegacyStatusBarComponent],
  templateUrl: './repmznyh.component.html',
  styleUrl: './repmznyh.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RepmznyhComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('REPMZNYH');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/DATAKMZ?limit=500' + where + '&orderBy=NOS DESC'));
