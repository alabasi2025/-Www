...
@Component({
  selector: 'app-rep-amlh-snf',
  imports: [CommonModule, DecimalPipe, DatePipe, FormsModule, LegacyToolbarComponent, LegacyStatusBarComponent],
  templateUrl: './rep-amlh-snf.component.html',
  styleUrl: './rep-amlh-snf.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RepAmlhSnfComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('REP_AMLH_SNF');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/FB?limit=500' + where + '&orderBy=NOS DESC'));
