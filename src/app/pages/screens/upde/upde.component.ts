...
@Component({
  ...
  templateUrl: './upde.component.html',
  styleUrl: './upde.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UpdeComponent implements OnInit {
  private http = inject(HttpClient);
  private perms = inject(PermissionService);
  readonly p = this.perms.forScreen('UPDE');
  ...
  async loadList() {
    ...
    try {
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/USEIN?limit=500&orderBy=NOS DESC'));
  ...
  async loadOne(nos: number) {
    this.loading.set(true);
    try {
      const mR = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>(`/api/data/USEIN?where=NOS=${nos}&limit=1`));
      if (mR.ok && mR.rows[0]) {
  ...
  async save() {
    ...
    try {
      ...
      const r = await firstValueFrom(isNew ? this.http.post<{ok:boolean;error?:string}>('/api/data/USEIN',body) : this.http.put<{ok:boolean;error?:string}>('/api/data/USEIN',{...body,_where:'NOS='+this.masterRaw()['NOS']}));
  ...
  async del() {
    ...
    try { await firstValueFrom(this.http.delete('/api/data/USEIN?where=NOS='+nos)); await this.loadList(); }
    catch(e){this.err.set(e instanceof Error?e.message:'خطأ');} this.saving.set(false);
  }
