...
export class TsmComponent implements OnInit {
  ...
  async loadList() {
    ...
    try {
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/TSM?limit=500&orderBy=NOS DESC'));
  ...
  async loadOne(nos: number) {
    this.loading.set(true);
    try {
      const mR = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>(`/api/data/TSM?where=NOS=${nos}&limit=1`));
      if (mR.ok && mR.rows[0]) {
        ...
        const dR = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>(`/api/data/TSMF?where=NOS=${nos}&orderBy=RECNO`)); this.details.set(dR.rows||[]);
  ...
  async save() {
    ...
    try {
      ...
      const r = await firstValueFrom(isNew ? this.http.post<{ok:boolean;error?:string}>('/api/data/TSM',body) : this.http.put<{ok:boolean;error?:string}>('/api/data/TSM',{...body,_where:'NOS='+this.masterRaw()['NOS']}));
  ...
  async del() {
    ...
    try { await firstValueFrom(this.http.delete('/api/data/TSM?where=NOS='+nos)); await this.loadList(); }
    catch(e){this.err.set(e instanceof Error?e.message:'خطأ');} this.saving.set(false);
  }
