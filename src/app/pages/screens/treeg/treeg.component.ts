...
@Component({
  ...
  templateUrl: './treeg.component.html',
  styleUrl: './treeg.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TreegComponent implements OnInit {
  private http = inject(HttpClient);
  private perms = inject(PermissionService);
  readonly p = this.perms.forScreen('TREEG');
  ...
  async loadList() {
    ...
    try {
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/DATA_AG?limit=500&orderBy=NOS DESC'));
  ...
  async loadOne(nos: number) {
    this.loading.set(true);
    try {
      const mR = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>(`/api/data/DATA_AG?where=NOS=${nos}&limit=1`));
      if (mR.ok && mR.rows[0]) {
  ...
  async save() {
    ...
    try {
      ...
      const r = await firstValueFrom(isNew ? this.http.post<{ok:boolean;error?:string}>('/api/data/DATA_AG',body) : this.http.put<{ok:boolean;error?:string}>('/api/data/DATA_AG',{...body,_where:'NOS='+this.masterRaw()['NOS']}));
  ...
  async del() {
    ...
    try { await firstValueFrom(this.http.delete('/api/data/DATA_AG?where=NOS='+nos)); await this.loadList(); }
    catch(e){this.err.set(e instanceof Error?e.message:'خطأ');} this.saving.set(false);
  }
