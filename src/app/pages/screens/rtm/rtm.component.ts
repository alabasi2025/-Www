...
@Component({
  ...
  styleUrl: './rtm.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RtmComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('RTM');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/TM?limit=500' + where + '&orderBy=NOS DESC'));
