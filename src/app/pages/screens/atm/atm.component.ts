...
export class AtmComponent implements OnInit {
  ...
  readonly p = this.perms.forScreen('ATM');

  readonly model = signal({ DATES: '', NOA: 0, NAMES: '', MEMOS1: '', NOAML: 1, SARSF: 1, MRT: 0, TYPEMS: 10 });
  ...
  async loadOne(nos: number) {
    ...
    try {
      ...
      if (mR.ok && mR.rows[0]) {
        ...
        this.model.set({ DATES: String(m['DATES']??'').slice(0,10), NOA: Number(m['NOA']??0), NAMES: String(m['NAMES']??m['NAMEA']??''), MEMOS1: String(m['MEMOS1']??''), NOAML: Number(m['NOAML']??1), SARSF: Number(m['SARSF']??1), MRT: Number(m['MRT']??0), TYPEMS: 10 });
        const dR = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>(`/api/data/ATMF?where=NOS=${nos}&orderBy=RECNO`)); this.details.set(dR.rows||[]);
  ...
  onToolbarAction(action: LegacyToolbarActionId) {
    switch(action) {
      case 'new': this.mode.set('new'); this.model.set({DATES:new Date().toISOString().slice(0,10),NOA:0,NAMES:'',MEMOS1:'',NOAML:1,SARSF:1,MRT:0,TYPEMS:10}); this.masterRaw.set({}); this.details.set([]); this.clearMessages(); break;
