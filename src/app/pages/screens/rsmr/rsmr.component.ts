...
@Component({
  ...
  templateUrl: './rsmr.component.html',
  styleUrl: './rsmr.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RsmrComponent {
  private http = inject(HttpClient);
  readonly p = inject(PermissionService).forScreen('RSMR');
  ...
  async runReport() {
    ...
    try {
      ...
      const r = await firstValueFrom(this.http.get<{ok:boolean;rows:Row[]}>('/api/data/SMR?limit=500' + where + '&orderBy=NOS DESC'));
